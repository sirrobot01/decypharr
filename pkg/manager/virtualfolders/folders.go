...
// Compile returns valid folders and any errors in the definitions.
func Compile(definitions []config.VirtualFolder) (*Folders, error) {
	compiled := &Folders{
...
nextFolder:
	for _, definition := range definitions {
		definition.Conditions = slices.Clone(definition.Conditions)
		config.NormalizeVirtualFolder(&definition)
		if err := config.ValidateVirtualFolder(definition, nil); err != nil {
			errs = append(errs, fmt.Errorf("virtual folder %q: %w", definition.Name, err))
			continue nextFolder
		}
...
func (cf *Folders) Names() []string {
	...
	return slices.Clone(cf.folders)
}

func (cf *Folders) Has(folderName string) bool {
	if cf == nil {
		return false
	}
...
func (cf *Folders) IsTimeSensitive(folderName string) bool {
	if cf == nil {
		return false
	}
...
func (cf *Folders) Matches(folderName string, meta *storage.EntryMetaInfo, getFileNames func() []string) bool {
	folder, ok := cf.byName[folderName]
	if !ok || meta == nil {
		return false
	}
